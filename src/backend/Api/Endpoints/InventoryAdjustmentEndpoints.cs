...
public static class InventoryAdjustmentEndpoints
{
    ...
    {
        var adjustments = app.MapGroup("/api/inventory-adjustments").RequireAuthorization();
        adjustments.MapGet("/", ListAsync)
            .AddEndpointFilter(new OrganizationSetupEndpointFilter(true, OrganizationFeatureKeys.Inventory, OrganizationFeatureKeys.InventoryAdjustments))
            .AddEndpointFilter(new PermissionEndpointFilter(Permissions.InventoryStockLedgerView));
        adjustments.MapGet("/{id:guid}", GetAsync)
            .AddEndpointFilter(new OrganizationSetupEndpointFilter(true, OrganizationFeatureKeys.Inventory, OrganizationFeatureKeys.InventoryAdjustments))
            .AddEndpointFilter(new PermissionEndpointFilter(Permissions.InventoryStockLedgerView));
        adjustments.MapPost("/", CreateDraftAsync)
            .AddEndpointFilter(new OrganizationSetupEndpointFilter(true, OrganizationFeatureKeys.Inventory, OrganizationFeatureKeys.InventoryAdjustments))
            .AddEndpointFilter(new PermissionEndpointFilter(Permissions.InventoryAdjustmentCreate));
        adjustments.MapPut("/{id:guid}", UpdateDraftAsync)
            .AddEndpointFilter(new OrganizationSetupEndpointFilter(true, OrganizationFeatureKeys.Inventory, OrganizationFeatureKeys.InventoryAdjustments))
            .AddEndpointFilter(new PermissionEndpointFilter(Permissions.InventoryAdjustmentCreate));
        adjustments.MapDelete("/{id:guid}", DeleteDraftAsync)
            .AddEndpointFilter(new OrganizationSetupEndpointFilter(true, OrganizationFeatureKeys.Inventory, OrganizationFeatureKeys.InventoryAdjustments))
            .AddEndpointFilter(new PermissionEndpointFilter(Permissions.InventoryAdjustmentCreate));
        adjustments.MapPost("/{id:guid}/post", PostAsync)
            .AddEndpointFilter(new OrganizationSetupEndpointFilter(true, OrganizationFeatureKeys.Inventory, OrganizationFeatureKeys.InventoryAdjustments))
            .AddEndpointFilter(new PermissionEndpointFilter(Permissions.InventoryAdjustmentPost));
        adjustments.MapPost("/{id:guid}/cancel", CancelAsync)
            .AddEndpointFilter(new OrganizationSetupEndpointFilter(true, OrganizationFeatureKeys.Inventory, OrganizationFeatureKeys.InventoryAdjustments))
            .AddEndpointFilter(new PermissionEndpointFilter(Permissions.InventoryAdjustmentPost));
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await service.ListAsync(
            new InventoryAdjustmentListQuery(
                search,
                adjustmentNo,
                warehouseId,
                status,
    ...
    private static async Task<IResult> CreateDraftAsync(
        ...
        IValidator<UpsertInventoryAdjustmentRequest> validator,
        IInventoryAdjustmentService service,
        HttpContext context,
        CancellationToken cancellationToken)
    {
    ...
    private static async Task<IResult> PostAsync(
        Guid id,
        IInventoryAdjustmentPostingService service,
        HttpContext context,
        CancellationToken cancellationToken)
    {
    ...
    private static async Task<IResult> HandleDocumentActionAsync(Func<Task<InventoryAdjustmentDto?>> handler)
    {
        try
        {
            var result = await handler();
