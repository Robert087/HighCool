...
public sealed class BackupRetentionService(
    ...
    ILocalDatabaseOperationCoordinator operationCoordinator) : IBackupRetentionService
{
    public async Task<BackupRetentionResult> ApplyAsync(
        IReadOnlyCollection<string> activeBackupIds,
        CancellationToken cancellationToken)
    {
        var retentionOptions = await backupCatalogService.GetRetentionSettingsAsync(cancellationToken);
        ...
        var lease = await operationCoordinator.TryAcquireExclusiveAsync(
            LocalDatabaseOperationKind.Retention,
            null,
            cancellationToken);
        if (lease is null)
        {
            return new BackupRetentionResult(true, 0, 0, ["A local database operation is already in progress."]);
        }
                ...
                foreach (var group in manifests.GroupBy(item => item.Manifest.Reason))
                {
                    ...
                    foreach (var item in ordered.Skip(keepCount))
                    {
                        if (string.Equals(item.Manifest.BackupId, newest.Manifest?.BackupId, StringComparison.OrdinalIgnoreCase) ||
                            ...
                            operationCoordinator.ActiveBackupIds.Contains(item.Manifest.BackupId, StringComparer.OrdinalIgnoreCase) ||
    ...
    private static int GetKeepCount(BackupReason reason, BackupRetentionSettingsDto options)
